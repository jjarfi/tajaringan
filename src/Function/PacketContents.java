...
public class PacketContents implements PacketReceiver {
    ...
    public void receivePacket(Packet packet) {
        ...
        if (packet instanceof TCPPacket) {
            ...
            Object[] row = {M.No, tcp.length, tcp.src_ip, tcp.dst_ip, "TCP"};

            rowList.add(new Object[]{M.No, tcp.length, tcp.src_ip, tcp.dst_ip, "TCP", tcp.src_port, tcp.dst_port,
       ...
       //     DefaultTableModel model = (DefaultTableModel) sniffer.jTable1.getModel();
        //    model.addRow(row);
            ...
            Object[] row = {M.No, udp.length, udp.src_ip, udp.dst_ip, "UDP"};
            rowList.add(new Object[]{M.No, udp.length, udp.src_ip, udp.dst_ip, "UDP", udp.src_port, udp.dst_port,
     ...
     //       DefaultTableModel model = (DefaultTableModel) sniffer.jTable1.getModel();
       //     model.addRow(row);
            ...
            Object[] row = {M.No, icmp.length, icmp.src_ip, icmp.dst_ip, "ICMP"};
            rowList.add(new Object[]{M.No, icmp.length, icmp.src_ip, icmp.dst_ip, "ICMP", icmp.checksum, icmp.header,
