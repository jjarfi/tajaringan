...
import static Function.NetworkData.getRxChangeMap;
import static Function.NetworkData.getRxCurrentMap;
import static Function.NetworkData.getTxChangeMap;
import static Function.NetworkData.getTxCurrentMap;
...
public class I implements Initializable {
    ...
    private void netwok(ActionEvent e) {
        ...
        try {
            org.hyperic.sigar.NetInfo info = I.sigar.getNetInfo();
            NetInterfaceConfig config = I.sigar.getNetInterfaceConfig(null);
            txtip.setText(" = " + config.getAddress());
            txtgate.setText(" = " + config.getDestination());
            txtmac.setText(" = " + config.getHwaddr());
    ...
    private void updatesinyalauto(ActionEvent e) {
        try {
            String konek = Arrays.toString(sigar.getNetInterfaceList());
            // String info = sigar.get;
            txtname.setText("= " + konek);

        } catch (SigarException ex) {
            Logger.getLogger(I.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
